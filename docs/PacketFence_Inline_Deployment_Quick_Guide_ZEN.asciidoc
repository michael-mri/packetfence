...
PacketFence will use the domain and the hostname to generate the URL to redirect devices on the captive portal. If you have a HTTP certificate use the same hostname and domain name to validate the SSL connection on the captive portal.
...
It's possible to enable traffic shaping based on the role of the device.
In order to enable it you need to go in Confuration -> Network Configuration -> Inline Traffic Shaping and select the role you want to define a limit.
Set a upload and download speed limit and save.
